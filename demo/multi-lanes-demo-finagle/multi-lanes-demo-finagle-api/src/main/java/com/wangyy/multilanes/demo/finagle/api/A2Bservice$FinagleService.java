...
public class A2Bservice$FinagleService extends com.twitter.finagle.Service<byte[], byte[]> {
  final private A2Bservice.FutureIface iface;
  final private TProtocolFactory protocolFactory;

  public A2Bservice$FinagleService(final A2Bservice.FutureIface iface, final TProtocolFactory protocolFactory) {
    this.iface = iface;
    this.protocolFactory = protocolFactory;

    addFunction("a2b", new Function2<TProtocol, Integer, Future<byte[]>>() {
      public Future<byte[]> apply(TProtocol iprot, final Integer seqid) {
        try {
          A2b.Args args = A2b.Args.decode(iprot);
          iprot.readMessageEnd();
          Future<String> result;
          try {
            result = iface.a2b(args.msg);
          } catch (Throwable t) {
            result = Future.exception(t);
          }
          return result.flatMap(new Function<String, Future<byte[]>>() {
            public Future<byte[]> apply(String value){
              return reply("a2b", seqid, new A2b.Result.Builder().success(value).build());
            }
          ...
          try {
            iprot.readMessageEnd();
            return exception("a2b", seqid, TApplicationException.PROTOCOL_ERROR, e.getMessage());
