...
public class B2Cservice$FinagleClient implements B2Cservice.FutureIface {
    ...
    try {
      if (msg.type == TMessageType.EXCEPTION) {
        ...
        if (exception instanceof SourcedException) {
          if (this.serviceName != "") ((SourcedException) exception).serviceName_$eq(B2Cservice$FinagleClient.this.serviceName);
        }
        throw exception;
      } else {
  ...
  protected class __Stats {
    ...
    public __Stats(String name) {
      StatsReceiver scope = B2Cservice$FinagleClient.this.scopedStats.scope(name);
  ...
  private __Stats ___stats_b2c;
  
  private __Stats __stats_b2c() {
    if (___stats_b2c == null) {
      ___stats_b2c = new __Stats("b2c");
    }
    return ___stats_b2c;
  }
  ...
  public Future<String> b2c(String msg) {
    __stats_b2c().requestsCounter.incr();
  
    Future<String> rv = this.service.apply(encodeRequest("b2c", new B2c.Args(msg))).flatMap(new Function<byte[], Future<String>>() {
  
      public Future<String> apply(byte[] in) {
        try {
          B2c.Result result = decodeResponse(in, B2c.Result.CODEC);
  
  
  
          if (result.success.isDefined()) return Future.value(result.success.get());
          return Future.exception(missingResult("b2c"));
        } catch (TException e) {
      ...
      public Future<String> apply(Throwable t) {
        if (t instanceof SourcedException) {
          ((SourcedException) t).serviceName_$eq(B2Cservice$FinagleClient.this.serviceName);
        }
    ...
    rv.addEventListener(new FutureEventListener<String>() {
      public void onSuccess(String result) {
        __stats_b2c().successCounter.incr();
      }
      ...
      public void onFailure(Throwable t) {
        ...
        __stats_b2c().failuresCounter.incr();
        __stats_b2c().failuresScope.counter0(t.getClass().getName()).incr();
      }
    });
