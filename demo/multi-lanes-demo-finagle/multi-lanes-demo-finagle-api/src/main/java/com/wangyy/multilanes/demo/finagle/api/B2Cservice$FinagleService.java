...
public class B2Cservice$FinagleService extends com.twitter.finagle.Service<byte[], byte[]> {
  final private B2Cservice.FutureIface iface;
  final private TProtocolFactory protocolFactory;

  public B2Cservice$FinagleService(final B2Cservice.FutureIface iface, final TProtocolFactory protocolFactory) {
    this.iface = iface;
    this.protocolFactory = protocolFactory;

    addFunction("b2c", new Function2<TProtocol, Integer, Future<byte[]>>() {
      public Future<byte[]> apply(TProtocol iprot, final Integer seqid) {
        try {
          B2c.Args args = B2c.Args.decode(iprot);
          iprot.readMessageEnd();
          Future<String> result;
          try {
            result = iface.b2c(args.msg);
          } catch (Throwable t) {
            result = Future.exception(t);
          }
          return result.flatMap(new Function<String, Future<byte[]>>() {
            public Future<byte[]> apply(String value){
              return reply("b2c", seqid, new B2c.Result.Builder().success(value).build());
            }
          ...
          try {
            iprot.readMessageEnd();
            return exception("b2c", seqid, TApplicationException.PROTOCOL_ERROR, e.getMessage());
